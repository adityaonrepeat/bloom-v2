...
- **Auth** — email/password + Google OAuth, with transactional email.
...
For the full design — Talk matchmaking flow, Aastha streaming, the emotion system, and the data model — see **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**.
...
> ⚠️ Never commit your real `.env` files. Only the keys‑only `.env.example` templates belong in git.
